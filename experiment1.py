...
if __name__ == "__main__":
    ...
    model = vit_b_16(num_classes=200).to(device)
    ...
    for epoch in range(10, epochs):
        ...
        model.train()
        running_loss = 0.0
        for i, (img_full_size, img_half_size, labels) in enumerate(train_loader):
            with autocast():
                optimizer.zero_grad()
                #outputs = model_torchvision(img_full_size.to(device))
                outputs = model(img_full_size.to(device))
            ...
            if i % 25 == 0:
                ...
                writer.add_scalar("Training Loss (ViT torchvision)", loss.item(), epoch * len(train_loader) + i)
        torch.save(model.state_dict(), f"vit/tv_vit_{epoch}.pth")
        scheduler.step()
